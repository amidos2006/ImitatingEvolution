...
from .helper import get_range_reward, get_num_tiles, discretize, get_normalized_value
...
def fitness(genes, actions):
    ...
    regions = get_range_reward(number_regions, 1, 1, 0,\
                                genes.shape[0] * genes.shape[1] / 10)
    longest = get_range_reward(get_longest_path(genes, [1]), genes.shape[0] * genes.shape[1] / 2,\
...
def behaviors(genes, actions, bins):
    longest = discretize(get_normalized_value(get_longest_path(genes, [1]),\
